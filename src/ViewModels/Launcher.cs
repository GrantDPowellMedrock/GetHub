...
namespace GetHub.ViewModels
{
    public class Launcher : ObservableObject
    {
        ...
        public Launcher(string startupRepo)
        {
            ...
            // One-time: seed each workspace's groups from the sidebar folder tree.
            // Runs here (not in Preferences init) because prefs are writable now.
            pref.SeedWorkspaceGroupsFromTree();
        ...
        // Always get to the repositories view: focus an existing Welcome tab if
        // there is one, otherwise open a new one.
        public void OpenRepositoriesView()
        {
            foreach (var p in Pages)
            {
                if (p.Data is Welcome)
                {
                    ActivePage = p;
                    return;
                }
